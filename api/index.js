...
aws.config.update({
    secretAccessKey: "7Ygv9Y0585Ckp/TIgM7DhTslds7o2MJGJ/x6EoDd",
    accessKeyId: "AKIATFQYRWMC72FJWCL7",
    region: "ap-south-1",
...
const upload = multer({
    storage: multerS3({
        ...
        key: function (req, file, cb) {
            console.log(file);
            cb(null, 'uploadedfiles/' + file.fieldname + '-' + Date.now() + '.csv')
        }
    })
}).single('file');
...
async function UploadCsvDataToMySQL(job) {
    ...
    const getObjectParams = {
      ...
      const fileStream = 
    await new Promise(function (resolve, reject) {
        let stream = s3.getObject(getObjectParams).createReadStream();;
